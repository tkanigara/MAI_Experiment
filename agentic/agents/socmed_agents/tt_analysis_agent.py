from workflows.socmed_workflow.state import State, tiktok_result_analysis
from models.gemini import llm
from langchain_core.messages import HumanMessage, SystemMessage
from prompts.socmed_prompts_list.tt_analyst_prompt import KPI_PROMPT, SOCMED_OVERVIEW, FOLLOWERS_GROWTH, ENGAGEMENT_PERFORMANCE, TIKTOK_ANALYST, SYSTEM_PROMPT
from tools.tools_list_socmed.retrieval_tt_data import retrieve_kpi, retrieve_socmed_overview, retrieve_followers_growth, retrieve_engagement_performance, retrieve_content_by_bucket, retrieve_followers_growth_history, retrieve_engagement_performance_history, retrieve_competitor_analysis
...
def tt_analysis_agent(state: State) -> State:
    with node("Tiktok Agent analysis running"):
    #Kpi 
        ...
        tiktok_result = tiktok_result_analysis(
            client_code=state.Metadata.client_code,
...
def tt_analysis_agent_2nd(state: State) -> State:
    with node("Tiktok analysis running"):
