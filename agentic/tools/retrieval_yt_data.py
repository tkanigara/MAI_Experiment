    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            return {
                ...
                "overview": {
                    "client_id": str(row[0]),
                    "total_subscribers": to_number(row[1]),
                    "subscribers_lost": to_number(row[2]),
                    "total_posts": to_number(row[3]),
                    "video_posts": to_number(row[4]),
                    "shorts_posts": to_number(row[5]),
                    "likes": to_number(row[6]),
                    "comments": to_number(row[7]),
                    "total_views": to_number(row[8]),
                    "total_engagement": to_number(row[9]),
                    "engagement_rate": to_number(row[10])
                }
            }
        ...
        with conn.cursor() as cursor:
            ...
            return {
                ...
                "Followers growth": {
                    "client_id": str(row[0]),
                    "total_subscribers": to_number(row[1]),
                    "subscriber_growth": to_number(row[2]),
