...
export default function AdsClientDetailPage({ client, periods, onNavigate, onUpload, onOpenPeriod, onEditClient, onDeleteClient, onDeletePeriod }) {
  const platforms = adsPlatformFlags(client);
  const latestReport = periods[0]?.period_label || "-";
  const connectedSources = new Set(platforms.map((platform) => (["instagram", "facebook"].includes(platform) ? "meta" : platform))).size;
  return (
    <section className="view active">
      ...
      <section className="summary-strip">
        ...
        <div className="summary-item"><div className="summary-label">Data Sources</div><div className="summary-value">{connectedSources} configured</div></div>
      ...
      <div className="month-grid">
        {periods.length ? periods.map((period) => (
          <AdsMonthCard key={period.id} period={period} platforms={platforms} onOpen={() => onOpenPeriod(adsPeriodSlug(period))} onUpdate={onUpload} onDelete={onDeletePeriod} />
